...
func mainLoop(window: OpaquePointer, surface: WGPUSurface, device: WGPUDevice, queue: WGPUQueue, config: WGPUSurfaceConfiguration, renderPipeline: WGPURenderPipeline, bindGroup: WGPUBindGroup) {
    ...
    while true {
        ...
        drawFrame(surface: surface, device: device, queue: queue, config: config, renderPipeline: renderPipeline, bindGroup: bindGroup)
    }
}
